...
import com.monitor.model.PileTopHorizontalDisplacementData;
...
import com.monitor.view.PileTopHorizontalDisplacementPointSettingsController;
import com.monitor.controller.MainController;

/**
 * 桩顶水平位移数据管理控制器
 */
public class PileTopHorizontalDisplacementController {
    ...
    @FXML private TableView<PileTopHorizontalDisplacementData> dataTableView;
    @FXML private TableColumn<PileTopHorizontalDisplacementData, Number> serialNumberColumn;
    @FXML private TableColumn<PileTopHorizontalDisplacementData, String> pointCodeColumn;
    @FXML private TableColumn<PileTopHorizontalDisplacementData, Number> initialElevationColumn;
    @FXML private TableColumn<PileTopHorizontalDisplacementData, Number> previousElevationColumn;
    @FXML private TableColumn<PileTopHorizontalDisplacementData, Number> currentElevationColumn;
    @FXML private TableColumn<PileTopHorizontalDisplacementData, Number> currentChangeColumn;
    @FXML private TableColumn<PileTopHorizontalDisplacementData, Number> cumulativeChangeColumn;
    @FXML private TableColumn<PileTopHorizontalDisplacementData, Number> changeRateColumn;
    @FXML private TableColumn<PileTopHorizontalDisplacementData, String> mileageColumn;
    @FXML private TableColumn<PileTopHorizontalDisplacementData, Number> historicalCumulativeColumn;
    ...
    private ObservableList<PileTopHorizontalDisplacementData> dataList = FXCollections.observableArrayList();
    private Stage stage;
    
    // 所有测点的所有历史数据，结构为：<测点编号, <测量日期, 数据>>
    private Map<String, Map<LocalDate, PileTopHorizontalDisplacementData>> allPointDataMap = new HashMap<>();
    ...
    // 配置的测点列表
    private List<PileTopHorizontalDisplacementPoint> configuredPoints = new ArrayList<>();
    ...
    public void initialize() {
        ...
        // 初始化图表
        initializeCharts();
        showDisplacementChart(); // 默认显示位移变化图表
        
        // 初始化日期选择器为当前日期
        datePicker.setValue(LocalDate.now());
        
        // 添加表格数据变化监听器，当数据变化时自动更新图表
        dataList.addListener((javafx.collections.ListChangeListener.Change<? extends PileTopHorizontalDisplacementData> c) -> {
            updateChart();
        });
        ...
        // 加载测点配置
        loadPileTopHorizontalDisplacementPoints();
    }
    ...
    private void setupTableColumns() {
        ...
        initialElevationColumn.setCellFactory(column -> new TableCell<PileTopHorizontalDisplacementData, Number>() {
            @Override
        ...
        previousElevationColumn.setCellFactory(column -> new TableCell<PileTopHorizontalDisplacementData, Number>() {
            @Override
            protected void updateItem(Number item, boolean empty) {
                super.updateItem(item, empty);
                if (empty || item == null) {
        ...
        currentElevationColumn.setCellFactory(column -> new TableCell<PileTopHorizontalDisplacementData, Number>() {
            @Override
            protected void updateItem(Number item, boolean empty) {
                super.updateItem(item, empty);
                if (empty || item == null) {
        ...
        currentChangeColumn.setCellFactory(column -> new TableCell<PileTopHorizontalDisplacementData, Number>() {
            @Override
            protected void updateItem(Number item, boolean empty) {
                super.updateItem(item, empty);
                if (empty || item == null) {
        ...
        cumulativeChangeColumn.setCellFactory(column -> new TableCell<PileTopHorizontalDisplacementData, Number>() {
            @Override
            protected void updateItem(Number item, boolean empty) {
                super.updateItem(item, empty);
                if (empty || item == null) {
        ...
        // 变化速率列 - 单位为毫米/天，保留2位小数
        changeRateColumn.setCellValueFactory(cellData -> new SimpleDoubleProperty(cellData.getValue().getChangeRate()));
        changeRateColumn.setCellFactory(column -> new TableCell<PileTopHorizontalDisplacementData, Number>() {
            @Override
            protected void updateItem(Number item, boolean empty) {
                super.updateItem(item, empty);
                if (empty || item == null) {
        ...
        historicalCumulativeColumn.setCellFactory(column -> new TableCell<PileTopHorizontalDisplacementData, Number>() {
            @Override
            protected void updateItem(Number item, boolean empty) {
                super.updateItem(item, empty);
                if (empty || item == null) {
    ...
    private void initializeCharts() {
        ...
        displacementChart.setTitle("桩顶水平位移变化量");
     ...
     * 加载桩顶水平位移测点信息
     */
    private void loadPileTopHorizontalDisplacementPoints() {
        // 此处可以添加从文件或数据库加载测点配置的代码
        // 在实际应用中，应从配置文件或数据库加载测点数据
        // 这里仅添加示例测点数据
        if (configuredPoints.isEmpty()) {
            // 示例测点数据
            configuredPoints.add(new PileTopHorizontalDisplacementPoint("PH01", 100.500, "K0+100", 5.0, 10.0));
            configuredPoints.add(new PileTopHorizontalDisplacementPoint("PH02", 101.200, "K0+200", 5.0, 10.0));
            configuredPoints.add(new PileTopHorizontalDisplacementPoint("PH03", 102.100, "K0+300", 5.0, 10.0));
        }
    ...
    private void handleSettingsButtonAction(ActionEvent event) {
        try {
            // 加载设置对话框FXML
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/dialogs/PileTopHorizontalDisplacementPointSettingsDialog.fxml"));
            Parent root = loader.load();
            
            // 获取控制器
            PileTopHorizontalDisplacementPointSettingsController controller = loader.getController();
            ...
            // 获取更新后的测点列表
            List<PileTopHorizontalDisplacementPoint> updatedPoints = controller.getPoints();
        ...
        if (file != null) {
            try {
                ...
                try (FileInputStream fis = new FileInputStream(file);
                    ...
                    if (sheet == null) {
                        AlertUtil.showInformation("上传失败", "未找到含有'桩顶水平位移'的工作表，请检查Excel文件格式。");
                        return;
                    }
                    ...
                    if (!pointIdHeader.contains("测点") || !valueHeader.contains("测值")) {
                        AlertUtil.showInformation("上传失败", "表头格式不正确。需要包含'测点'和'测值'列。");
                        return;
                    }
                    ...
                    if (dataCount == 0) {
                        AlertUtil.showInformation("上传失败", "未找到有效的桩顶水平位移数据。");
                        return;
                    }
                    ...
                    List<PileTopHorizontalDisplacementData> displacementDataList = processExcelPointData(pointValueMap, measureDate);
                ...
                LocalDateTime dataBlockTimestamp = LocalDateTime.now();
                    ...
                    for (LocalDateTime timestamp : dataBlocksMap.keySet()) {
                        List<PileTopHorizontalDisplacementData> existingData = dataBlocksMap.get(timestamp);
                        if (!existingData.isEmpty() && existingData.get(0).getMeasurementDate().equals(measureDate)) {
                            // 有相同日期的数据，提示用户
                            handleDataDateConflict(measureDate, displacementDataList, file.getName(), dataCount);
                            return;
                        }
                    }
                ...
                addDataBlock(file.getName(), dataBlockTimestamp);
                    dataBlocksMap.put(dataBlockTimestamp, displacementDataList);
                ...
                // 添加自动保存功能
                autoSaveDataAfterUpload();
                
                // 显示成功消息
                    AlertUtil.showInformation("上传成功", "成功导入 " + dataCount + " 条桩顶水平位移数据。");
                }
            } catch (Exception e) {
     ...
     * 数据上传后自动保存
     */
    private void autoSaveDataAfterUpload() {
        try {
            // 获取当前控制器所在的标签页
            if (stage != null) {
                ...
                if (scene != null) {
                    ...
                    if (root instanceof BorderPane) {
                        ...
                        // 查找TabPane
                        TabPane tabPane = findTabPane(mainPane);
                        if (tabPane != null) {
                            // 找到当前标签页对应的MainController
                            Object controller = mainPane.getUserData();
                            if (controller instanceof MainController) {
                                MainController mainController = (MainController) controller;
                                // 调用MainController的自动保存方法
                                mainController.autoSaveCurrentProject();
                                System.out.println("数据上传后自动保存成功");
                            }
                        }
                    }
                }
            ...
            System.err.println("数据上传后自动保存失败: " + e.getMessage());
        }
    }
     ...
     * 处理数据日期冲突
     */
    private void handleDataDateConflict(LocalDate date, List<PileTopHorizontalDisplacementData> data, String fileName, int dataCount) {
     ...
     * 处理Excel点数据
     */
    private List<PileTopHorizontalDisplacementData> processExcelPointData(Map<String, Double> pointValueMap, LocalDate measureDate) {
        ...
        // 构建所有点的历史数据映射
        Map<String, Map<LocalDate, PileTopHorizontalDisplacementData>> allPointDataMap = getAllPointDataMap();
        
        // 遍历配置的测点
        for (PileTopHorizontalDisplacementPoint point : configuredPoints) {
            ...
            // 获取当前测值
            double currentElevation = pointValueMap.get(pointId);
            ...
            // 从历史数据中查找最近一次的前期数据
            Map<LocalDate, PileTopHorizontalDisplacementData> pointHistory = allPointDataMap.get(pointId);
            if (pointHistory != null && !pointHistory.isEmpty()) {
                ...
                if (previousDate != null) {
                    PileTopHorizontalDisplacementData previousData = pointHistory.get(previousDate);
                    previousElevation = previousData.getCurrentElevation();
                }
            }
            
            // 创建桩顶水平位移数据对象
            PileTopHorizontalDisplacementData displacementData = new PileTopHorizontalDisplacementData(
                    pointId, initialElevation, previousElevation, currentElevation, measureDate);
            
            // 设置里程
            displacementData.setMileage(point.getMileage());
            
            // 设置历史累计量
            displacementData.setHistoricalCumulative(point.getHistoricalCumulative());
     ...
     * 获取所有点的历史数据映射
     */
    private Map<String, Map<LocalDate, PileTopHorizontalDisplacementData>> getAllPointDataMap() {
        Map<String, Map<LocalDate, PileTopHorizontalDisplacementData>> result = new HashMap<>();
        
        // 遍历所有数据块
        for (LocalDateTime blockTime : dataBlocksMap.keySet()) {
            List<PileTopHorizontalDisplacementData> blockData = dataBlocksMap.get(blockTime);
            if (blockData != null) {
                for (PileTopHorizontalDisplacementData data : blockData) {
                    String pointId = data.getPointCode();
                    LocalDate date = data.getMeasurementDate();
                    
                    // 获取或创建该测点的数据映射
                    Map<LocalDate, PileTopHorizontalDisplacementData> pointDataMap = 
    ...
    private void updateDataDisplay() {
        ...
        if (selectedDataBlocks.size() == 1) {
            ...
            List<PileTopHorizontalDisplacementData> currentData = dataBlocksMap.get(currentDateTime);

            // 创建按测点排序的数据列表
            Map<String, PileTopHorizontalDisplacementData> dataMap = new HashMap<>();
            for (PileTopHorizontalDisplacementData data : currentData) {
                dataMap.put(data.getPointCode(), data);
            }

            // 按照测点列表顺序添加数据
            List<PileTopHorizontalDisplacementPoint> sortedPoints = new ArrayList<>(configuredPoints);
            
            // 按排序后的测点顺序添加数据
            for (PileTopHorizontalDisplacementPoint pointConfig : sortedPoints) {
                String pointCode = pointConfig.getPointId();
                PileTopHorizontalDisplacementData data = dataMap.get(pointCode);
            ...
            List<PileTopHorizontalDisplacementData> previousData = dataBlocksMap.get(previousDateTime);
            List<PileTopHorizontalDisplacementData> currentData = dataBlocksMap.get(currentDateTime);

            // 创建测点映射以便快速查找
            Map<String, PileTopHorizontalDisplacementData> previousDataMap = previousData.stream()
                    .collect(Collectors.toMap(PileTopHorizontalDisplacementData::getPointCode, data -> data));

            // 创建按测点排序的数据列表
            Map<String, PileTopHorizontalDisplacementData> currentDataMap = new HashMap<>();
            for (PileTopHorizontalDisplacementData data : currentData) {
            ...
            for (PileTopHorizontalDisplacementPoint pointConfig : sortedPoints) {
                String pointCode = pointConfig.getPointId();
                PileTopHorizontalDisplacementData currentItem = currentDataMap.get(pointCode);
                ...
                if (currentItem != null) {
                    ...
                    // 查找该测点的上一次记录
                    PileTopHorizontalDisplacementData previousItem = previousDataMap.get(pointCode);
    ...
    private void handleExportButtonAction(ActionEvent event) {
        ...
        fileChooser.setInitialFileName("桩顶水平位移数据.xlsx");
    ...
    public void loadFromMeasurementRecords(List<MeasurementRecord> records) {
        ...
        for (Map.Entry<LocalDate, List<MeasurementRecord>> entry : recordsByDate.entrySet()) {
            ...
            List<PileTopHorizontalDisplacementData> dataBlock = new ArrayList<>();
            ...
            for (MeasurementRecord record : dateRecords) {
                ...
                // 创建桩顶水平位移数据
                PileTopHorizontalDisplacementData data = new PileTopHorizontalDisplacementData();
    ...
    /**
     * 从桩顶水平位移数据存储对象加载数据
     * @param storage 桩顶水平位移数据存储对象
     */
    public void loadFromPileTopHorizontalDisplacementDataStorage(PileTopHorizontalDisplacementDataStorage storage) {
        if (storage == null) {
            return;
        }
        ...
        for (LocalDateTime timestamp : storage.getDataBlockTimestamps()) {
            List<PileTopHorizontalDisplacementData> dataList = storage.getDataBlock(timestamp);
    ...
    public List<MeasurementRecord> getMeasurementRecordsForSaving() {
        ...
        // 遍历所有数据块
        for (Map.Entry<LocalDateTime, List<PileTopHorizontalDisplacementData>> entry : dataBlocksMap.entrySet()) {
            for (PileTopHorizontalDisplacementData data : entry.getValue()) {
    ...
    public PileTopHorizontalDisplacementDataStorage getPileTopHorizontalDisplacementDataStorage() {
        PileTopHorizontalDisplacementDataStorage storage = new PileTopHorizontalDisplacementDataStorage();
        ...
        for (Map.Entry<LocalDateTime, List<PileTopHorizontalDisplacementData>> entry : dataBlocksMap.entrySet()) {
            ...
            List<PileTopHorizontalDisplacementData> dataBlock = entry.getValue();
