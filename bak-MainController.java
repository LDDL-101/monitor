...
public class MainController implements Initializable {
    ...
    private void openMonitoringItemTab(String itemName) {
        ...
        try {
            ...
            MonitoringItem item = monitoringItems.get(itemName);
                ...
                case "钢支撑轴力":
                    // 暂未实现钢支撑轴力模块
                    Label axialForceLabel = new Label(getItemType(itemName) + "监测模块 - 正在开发中");
                    axialForceLabel.setPadding(new Insets(20));
                    tab.setContent(axialForceLabel);
                    break;
                case "立柱竖向位移":
                    // 加载立柱竖向位移视图
                    ...
                    if (columnItem != null) {
                        ...
                        // 如果没有立柱竖向位移数据存储对象，但仍有旧版的PileDisplacementDataStorage，尝试使用它
                        else if (columnItem.getPileDisplacementDataStorage() != null) {
                            // 不再尝试从PileDisplacementDataStorage转换数据
                            // 立柱竖向位移应该使用自己的数据类型，而不是从桩顶位移转换
                            
                            // 创建立柱竖向位移的新存储对象
                            ColumnDisplacementDataStorage newStorage = columnItem.getOrCreateColumnDisplacementDataStorage();
                            
                            // 加载新的数据存储
                            columnController.loadFromDataStorage(newStorage);
                            System.out.println("为立柱竖向位移创建了新的独立数据存储对象");
                        }
                        }
    ...
    private String getItemType(String itemName) {
        // 沉降相关测项
        if (itemName.contains("地表点沉降")) return "地表点沉降";
        if (itemName.contains("立柱竖向位移")) return "立柱竖向位移"; 
        if (itemName.contains("建筑物沉降")) return "建筑物沉降";
        if (itemName.contains("沉降")) return "沉降";  // 兜底其他沉降类型
        
        // 位移相关测项
        if (itemName.contains("桩顶竖向位移")) return "桩顶竖向位移";
        if (itemName.contains("桩顶水平位移")) return "桩顶水平位移";
        if (itemName.contains("深部水平位移")) return "深部水平位移";
        if (itemName.contains("位移")) return "位移";  // 兜底其他位移类型
        
        // 倾斜相关测项
        if (itemName.contains("建筑物倾斜")) return "建筑物倾斜";
        if (itemName.contains("倾斜")) return "倾斜";  // 兜底其他倾斜类型
        
        // 应力相关测项
        if (itemName.contains("钢支撑轴力")) return "钢支撑轴力";
        if (itemName.contains("混凝土轴力")) return "混凝土轴力";
        if (itemName.contains("应力")) return "应力";  // 兜底其他应力类型
        
        // 水位相关测项
        if (itemName.contains("地表水位")) return "地表水位";
        if (itemName.contains("水位")) return "水位";  // 兜底其他水位类型
        
        // 裂缝相关测项
        if (itemName.contains("裂缝")) return "裂缝";
    ...
    private boolean hasUnsavedChanges(ProjectInfo project) {
        ...
        for (Tab tab : editorTabPane.getTabs()) {
            if (tab.getText().contains(project.getName()) && tab.getText().contains("地表点沉降")) {
                // 如果有打开的沉降数据标签页，则认为可能有未保存的更改
                return true;
            }
        }
    ...
    private void saveAllProjectData(ProjectInfo projectToSave) {
        ...
        for (String itemName : projectToSave.getMonitoringItems()) {
            ...
            if (!monitoringItems.containsKey(fullItemName)) {
                // 如果不存在，创建新的监测项
                MonitoringItem item = new MonitoringItem(
                    itemName.replaceAll("\\s+", ""),  // 移除空格作为ID
                    itemName,  // 名称
    ...
    private void collectSettlementDataFromOpenTabs(ProjectInfo project) {
        ...
        for (Tab tab : editorTabPane.getTabs()) {
            // 如果是地表点沉降标签页
            if (tab.getText().contains("地表点沉降") && tab.getContent() != null) {
                // 查找该项目的地表点沉降监测项
                String fullItemName = project.getName() + " - 地表点沉降";
                MonitoringItem subsidenceItem = monitoringItems.get(fullItemName);

                if (subsidenceItem == null) {
                    // 如果不存在，创建新的监测项
                    subsidenceItem = new MonitoringItem();
                    subsidenceItem.setId(UUID.randomUUID().toString());
                    subsidenceItem.setName("地表点沉降");
                    subsidenceItem.setType("沉降");
                    monitoringItems.put(fullItemName, subsidenceItem);
                }
                ...
                if (scrollPane != null) {
                    try {
                        // 尝试从userData获取控制器
                        Object controller = scrollPane.getUserData();

                        if (controller instanceof SettlementDataController) {
                            SettlementDataController settlementController = (SettlementDataController) controller;

                            // 获取数据并添加到监测项
                            List<MeasurementRecord> records = settlementController.getMeasurementRecordsForSaving();
                            if (records != null && !records.isEmpty()) {
                                // 清空之前的记录
                                subsidenceItem.setRecords(new ArrayList<>());

                                // 添加新记录
                                for (MeasurementRecord record : records) {
                                    subsidenceItem.addRecord(record);
                                }

                                System.out.println("已从沉降数据标签页收集 " + records.size() + " 条记录");
                            }

                            // 获取完整的沉降数据存储对象
                            SettlementDataStorage storage = settlementController.getSettlementDataStorage();
                            if (storage != null) {
                                // 将沉降数据存储对象保存到监测项中
                                subsidenceItem.setSettlementDataStorage(storage);
                                System.out.println("已保存沉降数据存储对象");
                            }
                        } else {
                            // 如果在userData中没有找到控制器，尝试递归查找子节点
                            findAndCollectFromController(scrollPane, project);
                        }
                    } catch (Exception e) {
                        // 如果获取controller出错，尝试递归查找子节点
                        findAndCollectFromController(scrollPane, project);
                    }
                }
            }
            // 如果是桩顶竖向位移标签页
            else if (tab.getText().contains("桩顶竖向位移") && tab.getContent() != null) {
                // 查找该项目的桩顶竖向位移监测项
                String fullItemName = project.getName() + " - 桩顶竖向位移";
                MonitoringItem pileDisplacementItem = monitoringItems.get(fullItemName);

                if (pileDisplacementItem == null) {
                    // 如果不存在，创建新的监测项
                    pileDisplacementItem = new MonitoringItem();
                    pileDisplacementItem.setId(UUID.randomUUID().toString());
                    pileDisplacementItem.setName("桩顶竖向位移");
                    pileDisplacementItem.setType("位移");
                    monitoringItems.put(fullItemName, pileDisplacementItem);
                }
                ...
                if (scrollPane != null) {
                    try {
                        ...
                        if (controller instanceof PileTopDisplacementController) {
                            PileTopDisplacementController pileDisplacementController = (PileTopDisplacementController) controller;

                            // 获取数据并添加到监测项
                            List<MeasurementRecord> records = pileDisplacementController.getMeasurementRecordsForSaving();
                            if (records != null && !records.isEmpty()) {
                                // 清空之前的记录
                                pileDisplacementItem.setRecords(new ArrayList<>());

                                // 添加新记录
                                for (MeasurementRecord record : records) {
                                    pileDisplacementItem.addRecord(record);
                                }

                                System.out.println("已从桩顶竖向位移标签页收集 " + records.size() + " 条记录");
                            }

                            // 获取完整的桩顶竖向位移数据存储对象
                            PileDisplacementDataStorage storage = pileDisplacementController.getSettlementDataStorage();
                            if (storage != null) {
                                // 将桩顶竖向位移数据存储对象保存到监测项中
                                pileDisplacementItem.setPileDisplacementDataStorage(storage);
                                System.out.println("已保存桩顶竖向位移数据存储对象");
                            }
                        } else {
                            // 如果在userData中没有找到控制器，尝试递归查找子节点
                            findAndCollectFromController(scrollPane, project);
    ...
    /**
     * 递归查找节点中的SettlementDataController或PileTopDisplacementController并收集数据
     */
    private void findAndCollectFromController(Parent parent, ProjectInfo project) {
        for (Node node : parent.getChildrenUnmodifiable()) {
            if (node instanceof Parent) {
                // 检查该节点的控制器
                if (node.getUserData() instanceof SettlementDataController) {
                    SettlementDataController controller = (SettlementDataController) node.getUserData();
                    String fullItemName = project.getName() + " - 地表点沉降";
                ...
                // 检查该节点是否为桩顶竖向位移控制器
                else if (node.getUserData() instanceof PileTopDisplacementController) {
                    PileTopDisplacementController controller = (PileTopDisplacementController) node.getUserData();
                    String fullItemName = project.getName() + " - 桩顶竖向位移";
                ...
                // 检查该节点是否为立柱竖向位移控制器
                else if (node.getUserData() instanceof ColumnDisplacementController) {
                    ColumnDisplacementController controller = (ColumnDisplacementController) node.getUserData();
                    String fullItemName = project.getName() + " - 立柱竖向位移";

                    // 获取或创建监测项
                    MonitoringItem columnDisplacementItem = monitoringItems.get(fullItemName);
                    if (columnDisplacementItem == null) {
                        columnDisplacementItem = new MonitoringItem();
                        columnDisplacementItem.setId(UUID.randomUUID().toString());
                        columnDisplacementItem.setName("立柱竖向位移");
                        columnDisplacementItem.setType("位移");
                        monitoringItems.put(fullItemName, columnDisplacementItem);
                    }

                    // 获取完整的立柱竖向位移数据存储对象
                    ColumnDisplacementDataStorage storage = controller.getColumnDisplacementDataStorage();
                    if (storage != null) {
                        // 将立柱竖向位移数据存储对象保存到监测项中
                        columnDisplacementItem.setColumnDisplacementDataStorage(storage);
                        System.out.println("已保存立柱竖向位移数据存储对象");
                    }
                }

                // 递归检查子节点
                findAndCollectFromController((Parent) node, project);
            }
        }
    }
    ...
    private void openMonitoringItemWithProject(String projectName, String itemName) {
        ...
        if (item == null) {
            ...
            item = new MonitoringItem(
                    itemName.replaceAll("[^A-Za-z0-9]", ""),  // 提取ID
                    itemName,  // 名称
                    getItemType(itemName),  // 类型
                    projectName.isEmpty() ? "未知位置" : projectName + " - " + itemName  // 位置
            );
            monitoringItems.put(getFullItemName(projectName, itemName), item);
            System.out.println("创建新的监测项: " + fullItemName);
        }
        ...
        try {
            ...
            switch (getItemType(itemName)) {
                ...
                case "位移":
                    ...
                    // 如果存在相应的监测项，加载其数据
                    MonitoringItem displacementItem = monitoringItems.get(fullItemName);
                    if (displacementItem != null) {
                        // 如果存在桩顶竖向位移数据存储对象，优先使用它
                        if (displacementItem.getPileDisplacementDataStorage() != null) {
                            displacementController.loadFromSettlementDataStorage(displacementItem.getPileDisplacementDataStorage());
                ...
                case "倾斜":
                    // 暂未实现倾斜模块
                    Label tiltLabel = new Label(getItemType(itemName) + "监测模块 - 正在开发中");
                    tiltLabel.setPadding(new Insets(20));
                    tab.setContent(tiltLabel);
                    break;
                case "地表水位":
                case "水位":
                    // 暂未实现水位模块
                    Label waterLevelLabel = new Label(getItemType(itemName) + "监测模块 - 正在开发中");
                    waterLevelLabel.setPadding(new Insets(20));
                    tab.setContent(waterLevelLabel);
                    break;
                case "立柱竖向位移":
                    // 加载立柱竖向位移视图
                    FXMLLoader columnLoader = new FXMLLoader(getClass().getResource("/fxml/ColumnDisplacementView.fxml"));
            ...
            System.err.println("加载监测项视图时出错: " + e.getMessage());
            e.printStackTrace();

            // 使用通用监测项编辑器作为备选
            MonitoringItemEditor editor = new MonitoringItemEditor(item);
     ...
     * 将测量记录转换为立柱位移数据列表
     */
    private List<PileDisplacementData> convertMeasurementRecordsToPileDisplacementData(List<MeasurementRecord> records) {
     ...
     * 将桩顶位移测点转换为立柱位移测点
     */
    private ColumnDisplacementPoint convertToColumnDisplacementPoint(PileDisplacementPoint pilePoint) {
        ColumnDisplacementPoint columnPoint = new ColumnDisplacementPoint(
            pilePoint.getPointId(),
            pilePoint.getInitialElevation(),
            pilePoint.getMileage(),
            pilePoint.getRateWarningValue(),
            pilePoint.getAccumulatedWarningValue(),
            pilePoint.getHistoricalCumulative()
        );
        columnPoint.setOrderIndex(pilePoint.getOrderIndex());
        return columnPoint;
    }
     ...
     * 将桩顶位移数据转换为立柱位移数据
     */
    private ColumnDisplacementData convertToColumnDisplacementData(PileDisplacementData pileData) {
        ColumnDisplacementData columnData = new ColumnDisplacementData(
            pileData.getPointCode(),
            pileData.getInitialElevation(),
            pileData.getPreviousElevation(),
            pileData.getCurrentElevation(),
            pileData.getMeasurementDate()
        );
        
        columnData.setCurrentChange(pileData.getCurrentChange());
        columnData.setCumulativeChange(pileData.getCumulativeChange());
        columnData.setChangeRate(pileData.getChangeRate());
        columnData.setMileage(pileData.getMileage());
        columnData.setHistoricalCumulative(pileData.getHistoricalCumulative());
