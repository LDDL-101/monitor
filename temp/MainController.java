...
public class MainController implements Initializable {
        ...
        overviewTreeView.setOnMouseClicked(event -> {
            if (event.getClickCount() == 2) {
                ...
                if (selectedItem != null && !selectedItem.getValue().equals("监测概况")) {
                    ...
                    // 处理预警信息点击
                    if (itemName.equals("预警信息")) {
                        // 找到所属项目
                        TreeItem<String> projectNode = selectedItem.getParent();
                        if (projectNode != null && !projectNode.getValue().equals("监测概况")) {
                            String projectName = projectNode.getValue();
                            openAlertInfoView(projectName);
                        }
                    }
                    // 处理统计数据点击
                    else if (itemName.equals("统计数据")) {
                        // 找到所属项目
                        TreeItem<String> projectNode = selectedItem.getParent();
                        if (projectNode != null && !projectNode.getValue().equals("监测概况")) {
                            // 可以添加打开项目统计数据的方法
                            // openProjectStatisticsView(projectNode.getValue());
                            showUndevelopedFeatureMessage("项目统计数据");
                        }
                    }
                    // 检查是否是项目节点
                    else if (!itemName.equals("监测概况") && selectedItem.getParent().getValue().equals("监测概况")) {
                        // 这是项目节点，不做特别处理，可以展开/折叠
                    } else {
                        // 处理其他情况，比如子功能模块
                        handleOverviewItemClick(itemName);
                    }
                }
            }
        });
     ...
     * 添加项目到工程列表和监测概况
     */
    private void addProjectToTreeViews(String projectName) {
        // 添加到工程列表
        TreeItem<String> projectRoot = projectTreeView.getRoot();
        if (projectRoot != null) {
            // 检查是否已存在，避免重复添加
            boolean exists = false;
            for (TreeItem<String> child : projectRoot.getChildren()) {
                if (child.getValue().equals(projectName)) {
                    exists = true;
                    break;
                }
            }
            ...
            if (!exists) {
                TreeItem<String> projectNode = new TreeItem<>(projectName);
                projectRoot.getChildren().add(projectNode);
            }
        }
        
        // 添加到监测概况
        TreeItem<String> overviewRoot = overviewTreeView.getRoot();
        if (overviewRoot != null) {
            // 检查是否已存在，避免重复添加
            boolean exists = false;
            for (TreeItem<String> child : overviewRoot.getChildren()) {
                if (child.getValue().equals(projectName)) {
                    exists = true;
                    break;
                }
            ...
            if (!exists) {
                ...
                projectNode.getChildren().addAll(alerts, statistics);
                overviewRoot.getChildren().add(projectNode);
            }
        }
    }
     ...
     * 从工程列表和监测概况中移除项目
     */
    private void removeProjectFromTreeViews(String projectName) {
        // 从工程列表移除
        TreeItem<String> projectRoot = projectTreeView.getRoot();
        if (projectRoot != null) {
            projectRoot.getChildren().removeIf(item -> item.getValue().equals(projectName));
        }
        
        // 从监测概况移除
        TreeItem<String> overviewRoot = overviewTreeView.getRoot();
        if (overviewRoot != null) {
            overviewRoot.getChildren().removeIf(item -> item.getValue().equals(projectName));
        }
    }
    ...
    private void handleNewProject() {
        ...
        projectDialog.setTitle("新建项目");
        
        // 显示对话框并获取结果
        ProjectInfo projectInfo = projectDialog.showAndWait().orElse(null);
        ...
        if (projectInfo != null) {
            ...
            // 添加到工程树视图
            addProjectToTreeViews(projectInfo.getName());
    ...
    private void loadProjectData(ProjectFileData data, File file) {
        if (data != null) {
            ...
            if (project != null) {
                ...
                // 添加到树视图
                addProjectToTreeViews(project.getName());
            
                // 加载监测项信息
                Map<String, MonitoringItem> loadedItems = data.monitoringItems;
                if (loadedItems != null) {
                    for (Map.Entry<String, MonitoringItem> entry : loadedItems.entrySet()) {
                        monitoringItems.put(entry.getKey(), entry.getValue());
                    }
                }
            }
        }
    ...
    private void handleRemoveProject(TreeItem<String> projectNode) {
        ...
        String projectName = projectNode.getValue();
        ProjectInfo project = findProjectByName(projectName);
        
        if (project != null) {
            boolean confirm = AlertUtil.showConfirmationDialog(
                "删除项目",
                "确定要删除项目 '" + projectName + "' 吗？此操作无法撤销。",
                "删除",
                "取消"
            );
            ...
            if (confirm) {
                ...
                for (Tab tab : editorTabPane.getTabs()) {
                    if (tab.getText().startsWith(projectName + " - ") || tab.getText().equals(projectName)) {
                ...
                for (String key : monitoringItems.keySet()) {
                ...
                for (String key : itemsToRemove) {
                    monitoringItems.remove(key);
                }
                
                // 从树视图中移除
                removeProjectFromTreeViews(projectName);
                
                // 从文件系统删除项目文件（可选）
                if (project.getProjectFile() != null && project.getProjectFile().exists()) {
                    boolean deleteFile = AlertUtil.showConfirmationDialog(
                        "删除项目文件",
                        "是否也要删除项目文件？",
                        "删除文件",
                        "保留文件"
                    ...
                    if (deleteFile) {
                        try {
                            Files.delete(project.getProjectFile().toPath());
                        } catch (IOException e) {
                            AlertUtil.showError("操作失败", "无法删除项目文件: " + e.getMessage());
                        }
                    }
                }
            }
     ...
     * 打开预警信息视图，带有指定项目名称
     */
    private void openAlertInfoView(String projectName) {
        try {
            // 加载FXML
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/AlertInfoView.fxml"));
            ...
            // 创建新标签页
            Tab tab = new Tab(projectName + " - 预警信息");
            tab.setContent(root);
            ...
            if (project != null) {
                // 创建预警数据映射
                Map<String, AlertDataStorage> alertDataMap = new HashMap<>();
                
                // 如果项目有预警数据存储，加载它
                if (project.getAlertDataStorage() != null) {
                    alertDataMap.put(projectName, project.getAlertDataStorage());
                } else {
                    // 如果没有，创建一个新的并设置到项目中
                    AlertDataStorage alertData = new AlertDataStorage();
                    project.setAlertDataStorage(alertData);
                    alertDataMap.put(projectName, alertData);
                }
                
                // 设置项目名称
                controller.setProjectName(projectName);
                controller.setProjectAlertDataMap(alertDataMap);
            }
            
        } catch (IOException e) {
            showErrorDialog("加载错误", "无法加载预警信息视图", e.getMessage());
        }
    }
     ...
     * 打开预警信息视图
     */
    private void openAlertInfoView() {
        try {
            // 加载FXML
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/AlertInfoView.fxml"));
            Parent root = loader.load();
            ...
            for (String projectName : projects.keySet()) {
                ProjectInfo project = projects.get(projectName);
                if (project != null && project.getAlertDataStorage() != null) {
                    alertDataMap.put(projectName, project.getAlertDataStorage());
                }
            }
    ...
    private void handleOpenProject() {
        // 创建文件选择器
        FileChooser fileChooser = new FileChooser();
        fileChooser.setTitle("打开项目文件");
        fileChooser.getExtensionFilters().addAll(
                new FileChooser.ExtensionFilter("项目文件", "*.jc"),
                new FileChooser.ExtensionFilter("所有文件", "*.*")
        );
        
        // 设置初始目录为默认项目目录
        File defaultProjectDir = new File(System.getProperty("user.home"), "MonitorProjects");
        if (defaultProjectDir.exists()) {
            fileChooser.setInitialDirectory(defaultProjectDir);
        }
        ...
        if (selectedFile != null) {
            try {
                ...
                AlertUtil.showError("打开失败", "无法打开项目文件: " + e.getMessage());
            }
        }
    }
    ...
    private void saveAllProjectData(ProjectInfo projectToSave) {
        // 收集所有打开的标签页中的数据
        for (Tab tab : editorTabPane.getTabs()) {
            // 检查标签页内容的userData，获取控制器实例
            Object content = tab.getContent();
            if (content != null && content instanceof Parent) {
                Parent parentContent = (Parent) content;
                Object userData = parentContent.getUserData();
                
                if (userData != null) {
                    // 根据控制器类型处理不同的数据保存
                    findAndCollectFromController(parentContent, projectToSave);
                }
            }
        }
    }
